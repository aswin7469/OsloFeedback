...
float lineMask(vec2 st, float width, float radius, vec2 maskHeight) {
    ...
    // The second factor is added to have only a line on the corner. That's what differentiate this
    // file from the glow_frag.glsl
    float circleLine = smoothstep(1. - 0.001 / r, 1., circle / r)
        * smoothstep(1. + 0.001 / radius, 1., circle / radius);
